...
const check = '<svg class="text-success"><use href="#fas-fa-circle-check" xlink:href="#fas-fa-circle-check"></use></svg>';
const x = '<svg class="text-danger"><use href="#fas-fa-circle-xmark" xlink:href="#fas-fa-circle-xmark"></use></svg>';
    ...
    for (let i = start; i < length; i++) {
        ...
        row.innerHTML = `
            ...
            ${(field === 'metadata' || field === 'url' || field === 'overview') && domain.status != 200 ?
                '<span title="Inaccessible (status ' + domain.status + ')"><svg class="text-warning"><use href="#fas-fa-circle-exclamation" xlink:href="#fas-fa-circle-exclamation"></use></svg></span>'
